...
public class PlayMidi {
	...
	public static void playMidi(List<Chord> chords) {
		...
		try {
            ...
            for (var chord : chords) {
            	for(var note:chord) {
            		// Add Note On event 144
            		 track.add(makeEvent(ShortMessage.NOTE_ON, 1, note, 120, ticksPerSecond * time));
            		 //ShortMessage.NOTE_OFF 128
            		 track.add(makeEvent(ShortMessage.NOTE_OFF, 1,note, 120, (ticksPerSecond * time) + ticksPerSecond)); 
            	}
            	time++;
            }        
